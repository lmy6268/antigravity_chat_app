...
export const registerRoomHandlers = (io: Server, socket: CustomSocket) => {
  // 방 참가 처리
  socket.on(CLIENT_EVENTS.JOIN_ROOM, async ({ roomId, username }: { roomId: string; username: string }) => {
    socket.join(roomId);
  ...
  // 방 나가기 알림
  socket.on(CLIENT_EVENTS.LEAVE_ROOM, async ({ roomId, username }: { roomId: string; username: string }) => {
    socket.leave(roomId);
