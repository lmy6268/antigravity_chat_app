...
const devConfig: ServerConfig = {
  ...
  ssl: {
    certPath:
      ...
      path.resolve(process.cwd(), 'src/server/certs/server.crt'),
    keyPath:
      process.env.SSL_KEY_PATH ||
      path.resolve(process.cwd(), 'src/server/certs/server.key'),
  },
};
...
export function getHttpsOptions() {
  ...
  if (fs.existsSync(resolvedCertPath) && fs.existsSync(resolvedKeyPath)) {
    try {
      ...
      serverLogger.error(
        `SSL 파일을 읽는 중 오류 발생: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  } else {
